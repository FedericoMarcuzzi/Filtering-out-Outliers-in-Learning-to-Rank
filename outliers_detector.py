...
class OutliersFinder():
    ...
    def __call__(self, y_score, data):
        if self.iteration >= self.start and data.params['name'] == 'train_set':
            if self.all_outleirs_ids is None:
                self.all_outleirs_ids = np.zeros(y_score.shape[0])
            self.last_outleirs_ids = self._compute_outliers_ids(data.label, y_score, data.group, self.cutoff)
            self.all_outleirs_ids[self.last_outleirs_ids] += 1
    ...
    def get_outliers_ids(self, p_sour=1, last_sour=False):
        if last_sour:
            return self.last_outleirs_ids
        
        return np.where(self.all_outleirs_ids >= int(p_sour * (self.end - self.start)))[0]
